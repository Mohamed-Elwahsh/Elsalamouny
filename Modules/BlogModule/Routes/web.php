...
Route::group(['prefix' => 'admin-panel', 'middleware' => ['auth:admin']], function(){
    ...
    Route::group(['middleware' => ['role:writer|admin|superadmin|owner']], function () {
        ...
        Route::get('blogs/ads','BlogController@adsDataTables')->name('ads');
        Route::get('blogs/active/{id}', 'BlogController@active')->name('active');
        Route::get('blogs/details/{id}', 'BlogController@details')->name('details');
